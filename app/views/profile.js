function renderProfileView(coreCard, profileIds, assumptionIds, eventsCard){
  ...
  profileCard.innerHTML = '<div class="section-head profile-hero"><div><span class="eyebrow">Profile input center</span><h3>Profile</h3><p>Profile is structurally stabilized as Bastion’s canonical intake workspace, but it is not feature-complete. Expanded household, income, asset, debt, tax, and special-account modeling remain future scoped work.</p></div><div class="profile-trust-stack"><span>Partial</span><span>Inputs owned here</span><span>Outputs stay on Dashboard</span><span>No formula changes</span></div></div><div class="view-status-banner"><span class="view-status-label status-partial">Partial</span><span>Profile owns baseline inputs. Current sections preserve future expansion paths without changing formulas, input IDs, or Dashboard output ownership.</span></div><div id="planSetupSummary" class="dashboard-summary profile-summary"><strong>Profile review checklist:</strong> Annual inputs are labeled “/ year,” monthly special-account placeholders are labeled “monthly,” and Dashboard remains output-only.</div><div class="profile-guidance-strip"><span>1. Confirm current household</span><span>2. Review annual cash flow</span><span>3. Check retirement assumptions</span></div><div class="mini-list profile-section-list" id="profileSections"></div>';
...
function ensureProfileCoreFields(grids,values){
  ...
  ensureProfileField(grids.scenarioAssumptions,'returnProfile','Return profile','<select id="returnProfile" onchange="commit()">'+optionHtml(opts.returnProfile,value('returnProfile','moderate'))+'</select>','Simple return assumption profile; formulas are unchanged in 2.40e.');
